...
import com.helpofai.hoa.musicplayer.adapter.album.AlbumAdapter
import com.helpofai.hoa.musicplayer.adapter.artist.ArtistAdapter
import com.helpofai.hoa.musicplayer.adapter.song.SongAdapter
import com.helpofai.hoa.musicplayer.fragments.home.HomeFragment
...
import com.helpofai.hoa.musicplayer.model.Home
...
class HomeAdapter(private val activity: AppCompatActivity) :
    ...
    companion object {
        private const val AD_SECTION = -1
    ...
    override fun getItemViewType(position: Int): Int {
        ...
        return list[getRealPosition(position)].homeSection
    }
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (getItemViewType(position) == AD_SECTION) {
            (holder as AdViewHolder).loadAd()
            return
        }
        val realPos = getRealPosition(position)
        if (realPos < 0 || realPos >= list.size) return

        val home = list[realPos]
