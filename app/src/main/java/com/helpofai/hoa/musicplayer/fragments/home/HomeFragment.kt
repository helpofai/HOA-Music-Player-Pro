...
import com.helpofai.hoa.musicplayer.adapter.HomeAdapter
import com.helpofai.hoa.musicplayer.ads.AdsManager
import com.helpofai.hoa.musicplayer.databinding.FragmentHomeBinding
import com.helpofai.hoa.musicplayer.dialogs.CreatePlaylistDialog
import com.helpofai.hoa.musicplayer.dialogs.ImportPlaylistDialog
import com.helpofai.hoa.musicplayer.extensions.accentColor
import com.helpofai.hoa.musicplayer.extensions.dip
import com.helpofai.hoa.musicplayer.extensions.elevatedAccentColor
...
class HomeFragment :
    ...
    private fun adjustPlaylistButtons() {
        val buttons =
            listOf(binding.history, binding.lastAdded, binding.topPlayed, binding.actionShuffle, binding.adButton)
    ...
    private fun setupListeners() {
        ...
        if (AdsManager.shouldShowAds()) {
            ...
            binding.adButton.setOnClickListener {
                AdsManager.showInterstitialAd(mainActivity)
    ...
    private fun setupTitle() {
        ...
        val appName = "hoa <font color=$hexColor>Music</font>".parseAsHtml()
    ...
    fun colorButtons() {
        ...
        binding.adButton.elevatedAccentColor()
    }
    ...
    private fun loadSuggestions(songs: List<Song>) {
        ...
        if (AdsManager.shouldShowAds()) {
            binding.suggestions.cardAd.isVisible = true
            AdsManager.getNativeAd(requireContext()) { nativeAd ->
                if (_binding == null) return@getNativeAd
                if (nativeAd != null) {
                    val adView = LayoutInflater.from(requireContext())
                        .inflate(R.layout.ad_album_cover, binding.suggestions.adContainer, false) as NativeAdView
                    AdsManager.populateNativeAdView(nativeAd, adView)
                    binding.suggestions.adContainer.removeAllViews()
                    binding.suggestions.adContainer.addView(adView)
                }
            }
            binding.suggestions.removeAds.setOnClickListener {
                findNavController().navigate(R.id.settings_fragment)
            }
        } else {
            binding.suggestions.cardAd.isVisible = false
        }
