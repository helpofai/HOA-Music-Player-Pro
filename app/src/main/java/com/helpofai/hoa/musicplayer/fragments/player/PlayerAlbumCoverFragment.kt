 ...
 *
 */
package com.helpofai.hoa.musicplayer.fragments.player
...
import com.helpofai.hoa.appthemehelper.util.MaterialValueHelper
...
import com.helpofai.hoa.musicplayer.adapter.album.AlbumCoverPagerAdapter
import com.helpofai.hoa.musicplayer.adapter.album.AlbumCoverPagerAdapter.AlbumCoverFragment
import com.helpofai.hoa.musicplayer.databinding.FragmentPlayerAlbumCoverBinding
...
import com.helpofai.hoa.musicplayer.lyrics.CoverLrcView
import com.helpofai.hoa.musicplayer.model.lyrics.Lyrics
import com.helpofai.hoa.musicplayer.transform.CarousalPagerTransformer
import com.helpofai.hoa.musicplayer.transform.ParallaxPagerTransformer
import com.helpofai.hoa.musicplayer.util.CoverLyricsType
import com.helpofai.hoa.musicplayer.util.LyricUtil
...
class PlayerAlbumCoverFragment : AbsMusicServiceFragment(R.layout.fragment_player_album_cover),
    ...
    private var nativeAdLoadGeneration = 0
    ...
    override fun onDestroyView() {
        super.onDestroyView()
        AdsManager.destroyNativeAd(currentNativeAd)
    ...
    private fun loadNativeAd() {
        if (!AdsManager.shouldShowAds()) {
            binding.adContainerAlbumCover.isVisible = false
            binding.viewPager.isVisible = true
            return
        }

        // Destroy previous native ad before loading a new one
        AdsManager.destroyNativeAd(currentNativeAd)
        currentNativeAd = null

        val generation = ++nativeAdLoadGeneration

        AdsManager.getNativeAd(requireContext()) { nativeAd ->
            // Guard against stale callbacks from rapid swiping
            if (_binding == null || generation != nativeAdLoadGeneration) {
                AdsManager.destroyNativeAd(nativeAd)
                return@getNativeAd
            }
            if (nativeAd != null) {
                currentNativeAd = nativeAd
                val adView = LayoutInflater.from(requireContext())
                    .inflate(R.layout.ad_album_cover, binding.adContainerAlbumCover, false) as NativeAdView
                AdsManager.populateNativeAdView(nativeAd, adView)
                binding.adContainerAlbumCover.removeAllViews()
                binding.adContainerAlbumCover.addView(adView)
                binding.adContainerAlbumCover.isVisible = true
