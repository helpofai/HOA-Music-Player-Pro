...
class PlaybackManager(val context: Context) {
        ...
        if (playback != null && !playback!!.isPlaying) {
            if (!playback!!.isInitialized) {
                ...
                if (playbackLocation == PlaybackLocation.LOCAL) {
                    if (playback is ExoCrossFadePlayer) {
                        if (!(playback as ExoCrossFadePlayer).isCrossFading) {
    ...
    fun maybeSwitchToCrossFade(crossFadeDuration: Int): Boolean {
        ...
        if (playback !is HoaExoPlayer && crossFadeDuration == 0) {
            ...
            return true
        } else if (playback !is ExoCrossFadePlayer && crossFadeDuration > 0) {
            if (playback != null) {
                playback?.release()
            }
            playback = null
            playback = ExoCrossFadePlayer(context)
            return true
        }
        return false
    }
