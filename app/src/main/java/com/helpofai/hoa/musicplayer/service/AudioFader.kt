...
class AudioFader {
    companion object {
        fun createFadeAnimator(
            context: Context,
            fadeInPlayer: Playback,
            fadeOutPlayer: Playback,
            ...
            return ValueAnimator.ofFloat(0f, 1f).apply {
                ...
                addUpdateListener { animation: ValueAnimator ->
                    fadeInPlayer.setVolume(
                        animation.animatedValue as Float
                    )
                    fadeOutPlayer.setVolume(1 - animation.animatedValue as Float)
                }
                doOnEnd {
                    endAction(it)
                }
        ...
        fun startFadeAnimator(
            ...
            if (duration == 0L) {
                playback.setVolume(if (fadeIn) 1.0f else 0.0f)
                callback?.run()
                return
            }
