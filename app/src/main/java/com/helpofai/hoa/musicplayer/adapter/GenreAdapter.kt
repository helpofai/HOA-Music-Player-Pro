...
class GenreAdapter(
    ...
    companion object {
        private const val GENRE_TYPE = 0
    ...
    override fun getItemId(position: Int): Long {
        if (getItemViewType(position) == AD_TYPE) return -(position.toLong() + 5000)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        if (viewType == AD_TYPE) {
            val view = LayoutInflater.from(activity).inflate(R.layout.item_genre_ad, parent, false)
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        val genre = dataSet[getRealPosition(position)]
        val genreHolder = holder as ViewHolder
        genreHolder.binding.title.text = genre.name
        genreHolder.binding.text.text = String.format(
            Locale.getDefault(),
            "%d %s",
            genre.songCount,
            if (genre.songCount > 1) activity.getString(R.string.songs) else activity.getString(R.string.song)
        )
        loadGenreImage(genre, genreHolder)
    }
    ...
    inner class ViewHolder(val binding: ItemGenreBinding) : RecyclerView.ViewHolder(binding.root),
        ...
        override fun onClick(v: View?) {
            ...
            if (realPos >= 0 && realPos < dataSet.size) {
                listener.onClickGenre(dataSet[realPos], itemView)
            }
        }
