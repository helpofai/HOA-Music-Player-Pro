...
import com.helpofai.hoa.musicplayer.databinding.FragmentMainSettingsBinding
import com.helpofai.hoa.musicplayer.extensions.drawAboveSystemBarsWithPadding
import com.helpofai.hoa.musicplayer.extensions.goToProVersion
...
class MainSettingsFragment : Fragment(), View.OnClickListener {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.buyPremium.setOnClickListener {
            if (!App.isProVersion() && AdsManager.shouldShowAds()) {
                // Preload and show rewarded ad — user can earn a temporary Pro trial
                AdsManager.loadRewardedAd(requireContext())
                AdsManager.showRewardedAd(
                    requireActivity(),
                    onReward = { _, _ ->
                        Snackbar.make(binding.root, R.string.pro_trial_unlocked, Snackbar.LENGTH_LONG).show()
                    },
                    onError = {
                        requireContext().goToProVersion()
                    }
